...
interface Hooks {
   ...
   */
  afterUpload?: (req: ExpressRequest, file: MulterFile, fileRef: FirebaseFile, bucketRef: FirebaseBucket) => void
  /**
   ...
   */
  afterDelete?: (req: ExpressRequest, file: MulterFile, fileRef: FirebaseFile, bucketRef: FirebaseBucket) => void
  /**
...
class FirebaseStorage {
  ...
  #mimeMap: MimeMap = {}
  #hooks: Hooks = {}
  #sharpPipeline: sharp.Sharp | null = null

  #required = (message: string) => {
    throw new Error(message)
  }

  #callHook<HookName extends keyof typeof AvailableHooks>(
    hookName: HookName,
    ...params: Parameters<Required<Hooks>[HookName]>
  ): void {
    ...
    if (hookToBeCalled) {
      return (hookToBeCalled as unknown as HookFunction).call<ThisType<FirebaseStorage>, HookParameters, HookReturn>(
        this,
        ...params
      )
    }
  ...
  constructor(opts: MulterFirebaseOptions, firebaseClient: fbAdmin.app.App | null) {
    ...
    this.#sharpPipeline = opts.sharpPipeline || null
  ...
  _handleFile(
    req: ExpressRequest,
    file: MulterFile,
    cb: (err: Error | null, info?: MulterFirebaseStorageFileReference) => void
  ) {
    ...
    if (this.#sharpPipeline) {
      file.stream.pipe(this.#sharpPipeline).pipe(outStream)
    } else {
  ...
  #getFileName(file: MulterFile) {
    return `${this.#directoryPath ? this.#directoryPath + '/' : ''}${this.#namePrefix}${
      file.originalname.split('.')[0]
    }${this.#nameSuffix}${this.#unique ? Date.now().toString(16) : ''}.${file.originalname.split('.')[1] || ''}`
  }
  ...
  #validateCredentials(credentials: string | FirebaseCredentials) {
    ...
    if (!['string', 'object'].includes(typeof credentials))
      return this.#required('Credentials must be a string or service account object')
    if (
      (typeof credentials === 'object' && !(credentials as FirebaseCredentials).projectId) ||
      !(credentials as FirebaseCredentials).privateKey ||
      !(credentials as FirebaseCredentials).clientEmail
    )
      return this.#required('Credentials must be a string or service account object')
    return credentials
  }
...
module.exports = (opts: MulterFirebaseOptions, firebaseClient: Nullable<fbAdmin.app.App> = null) =>
  new FirebaseStorage(opts, firebaseClient)
